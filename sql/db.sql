...
-- Model: Sistema Noleggio    Version: 1.0
...
CREATE TABLE IF NOT EXISTS `sistema_noleggio`.`carica` (
  ...
  INDEX `modifica_carica_corrente` (`impiegato`, `fine`),
...
CREATE TABLE IF NOT EXISTS `sistema_noleggio`.`noleggio` (
  ...
  INDEX `ricerca_noleggio_corrente` (`restituzione`, `scadenza`),
...
SELECT impiegato, mansione FROM carica WHERE (fine IS NULL AND CURDATE() >= inizio) OR (CURDATE() BETWEEN inizio AND fine);
...
BEGIN
	...
	IF New.fine < CURDATE() THEN
		SIGNAL SQLSTATE '45000' SET message_text = "La data di fine deve essere maggiore o uguale della data odierna";
    END IF;
...
CREATE USER 'sn_cassiere' IDENTIFIED BY 'sn_cassiere';
...
GRANT EXECUTE ON procedure `sistema_noleggio`.`registraNoleggio` TO 'sn_cassiere';
...
SET SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
CREATE USER 'sn_impiegato' IDENTIFIED BY 'sn_impiegato';
...
SET SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
CREATE USER 'sn_login' IDENTIFIED BY 'sn_login';
...
INSERT INTO `sistema_noleggio`.`film` (`titolo`, `regista`, `anno`, `costo_dvd`, `costo_videocassetta`, `originale_titolo`, `originale_regista`) VALUES ('La vita è bella', 'Roberto Benigni', 1997, 3.59, 2.59, NULL, NULL);
...
DROP EVENT IF EXISTS `sistema_noleggio`.`cleanup_10`;
...
DROP EVENT IF EXISTS `sistema_noleggio`.`cleanup_5`;
